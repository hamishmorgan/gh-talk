# pr-iterate

Iterating on Pull Requests

**Purpose**: Handle CI monitoring, review feedback, and iterative improvements on an open PR.

**When to Use**: After PR created, during review cycle

**Previous Step**: PR created via `/pr-create`

**Prerequisites**:
- Open PR on feature branch
- Copilot review requested
- CI running or completed

## Step 0: Check if Rebase Needed (DO THIS FIRST!)

**CRITICAL**: ALWAYS check and rebase BEFORE investigating failures. Many CI issues (formatting, linting, config) are already fixed in main.

```bash
# Check if PR is behind main
gh pr view <PR> --json mergeStateStatus --jq '.mergeStateStatus'

# If "BEHIND", rebase IMMEDIATELY:
gh pr checkout <PR>
git fetch origin main
git rebase origin/main
# Fix conflicts if any
git push --force-with-lease

# Re-check CI after rebase
gh pr checks <PR>
```

**Why rebase first?**
- Main branch may have recent fixes (golangci-lint config, formatting rules)
- Rebasing brings in those fixes automatically
- Saves time investigating issues that are already solved
- Ensures you're working with latest codebase state

**After rebasing:**
- CI may pass immediately without any fixes needed
- Only investigate failures that persist after rebase

## Monitor CI and Fix Failures

**Check CI status (exit code 0 = ALL pass):**

```bash
gh pr checks <PR>
# Exit code 0 = all pass
# Exit code 1 = failures or pending
```

**If ANY checks fail:**

```bash
# View failed logs
gh run view --log-failed

# Fix the issue
# Commit and push fix
# Wait for CI re-run

# NEVER claim "ready" until this returns exit code 0:
gh pr checks <PR>
```

**Watch run in real-time:**

```bash
gh run watch
```

**CRITICAL**: Do NOT proceed until `gh pr checks` returns exit code 0

## Address Review Feedback

**Check ALL comment types:**

```bash
# 1. General PR comments
gh pr view <PR> --comments

# 2. Inline review comments (use gh-talk!)
gh talk list threads --pr <PR>

# 3. Review decision
gh pr view <PR> --json reviewDecision --jq '.reviewDecision'
```

**For EACH comment/thread:**

### 1. Respond BEFORE Fixing

```bash
gh talk reply PRRT_xxx "👀 Looking into this

🤖 Cursor"
```

### 2. Make the Fix

- Implement the change
- Write tests (100% coverage for new code)
- Update docs if needed

### 3. Test the Fix

```bash
make fmt  # Format first
make ci   # Verify locally
```

### 4. Commit and Push

```bash
git add <files>
git commit --author="Cursor <cursor@noreply.local>" -m "Address review comment about X

Fixed by doing Y.

🤖 Generated by Cursor"

git push

# IMMEDIATELY REQUEST RE-REVIEW (REQUIRED)
# TODO: Find programmatic method - currently must use GitHub UI
# Go to PR and click "Request re-review" button next to Copilot's review
```

**CRITICAL**: ALWAYS request Copilot re-review immediately after pushing changes

**Current limitation**: No known programmatic method to request re-review from Copilot.
Must manually click "Request re-review" in GitHub UI after pushing.

**Investigated methods that DON'T work:**
- ❌ `gh pr edit --add-reviewer copilot` (not a collaborator)
- ❌ `gh pr comment "@github-copilot please re-review"` (doesn't trigger)
- ❌ `gh api .../requested_reviewers` (API 422 error)

**If you find a programmatic method, please update this workflow and create PR.**

### 5. Respond AFTER Fixing

```bash
gh talk reply PRRT_xxx "Fixed in latest commit. [Brief explanation]

🤖 Cursor" --resolve --react 👍
```

### 6. Clean Up

```bash
# Hide original comment after resolving
gh talk hide PRRC_xxx --reason resolved
```

### 7. Post Status Updates (If Significant Progress)

**ALWAYS hide old status updates first:**

```bash
# Hide all previous status updates
gh pr view <PR> --comments --json comments | \
  jq -r '.comments[] | select(.body | contains("Status Update")) | .id' | \
  xargs -I {} gh talk hide {} --reason outdated

# Then post new update
gh pr comment <PR> --body "## Status Update

✓ Items completed
⏳ Items in progress (e.g., CI running)
❌ Items blocked

🤖 Cursor"
```

**CRITICAL**: ALWAYS hide old updates before posting new one

**Emoji Usage:** See `/emoji-semantics` for complete guide

**Standard pattern:**
- 👀 Before fixing (acknowledging)
- 👍 After fixing (confirmation)
- ❤️ For helpful suggestions
- 🎉 For great feedback
- 😕 If unclear (ask clarification)

## Keep PR Updated

**Before each push, check if behind:**

```bash
gh pr view <PR> --json mergeStateStatus --jq '.mergeStateStatus'

# If "BEHIND", rebase:
git fetch origin main
git rebase origin/main
# Fix conflicts if any
git push --force-with-lease
```

**Update PR description when:**
- Scope expanded (more files changed)
- Approach changed significantly
- New issues closed
- Significant commits added

```bash
gh pr edit <PR> --body "Updated description

...

---
🤖 *Generated by Cursor*"
```

## Common Patterns

### Quick Fix Cycle

```bash
# For each comment:
gh talk reply PRRT_xxx "👀 Working on this

🤖 Cursor"

# Fix, test, commit, push
make fmt && make ci
git commit --author="Cursor <cursor@noreply.local>" -m "Fix X

🤖 Generated by Cursor"
git push

# Respond and resolve
gh talk reply PRRT_xxx "Fixed in this commit

🤖 Cursor" --resolve --react 👍

gh talk hide PRRC_xxx --reason resolved
```

### Rebase and Continue

```bash
git fetch && git rebase origin/main
git push --force-with-lease

# IMMEDIATELY REQUEST RE-REVIEW (REQUIRED)
gh pr comment <PR> --body "@github-copilot please re-review"
```

## Anti-Patterns

❌ **NOT rebasing first** - ALWAYS rebase before investigating CI failures
❌ **Claiming ready while CI failing** - Check exit code!
❌ **Accumulating fixes** - Push after each fix
❌ **Forgetting to respond** - Always acknowledge before fixing
❌ **NOT requesting re-review after push** - REQUIRED immediately after git push
❌ **Skipping gh talk** - Use our own tool!

## Quick Reference

```bash
# STEP 0: Rebase if behind (DO THIS FIRST!)
gh pr view <PR> --json mergeStateStatus --jq '.mergeStateStatus'
# If "BEHIND":
gh pr checkout <PR> && git fetch && git rebase origin/main && git push --force-with-lease

# Check CI (wait for exit code 0)
gh pr checks <PR>

# Address feedback
gh talk list threads
gh talk reply PRRT_xxx "👀 Looking

🤖 Cursor"
# fix
make fmt && make ci && git push

# IMMEDIATELY re-review (REQUIRED)
gh pr comment <PR> --body "@github-copilot please re-review"

# Then respond
gh talk reply PRRT_xxx "Fixed

🤖 Cursor" --resolve --react 👍
gh talk hide PRRC_xxx --reason resolved
```

## Next Steps

**When all feedback addressed and CI passing:**
→ Use `/pr-merge` for final review and merge request

---

**Related**:
- `pr-create` - Creating the PR
- `pr-merge` - Final review and merge
- `AGENTS.md` - Code standards

---
Note: This content duplicated in .cursor/rules/pr-iterate.mdc for auto-loading
