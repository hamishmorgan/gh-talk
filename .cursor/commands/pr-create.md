...
- **Commit messages**: `--author` flag + `🤖 Generated by Cursor` at end
...
- Go to PR page
- Click "Reviewers" → Add "copilot"

**Programmatic attempt (may not trigger review):**
```bash
gh pr comment <PR> --body "@github-copilot please review"
```

**Note**: Programmatic methods unreliable. Manually add Copilot as reviewer in GitHub UI to ensure review is requested.
...
gh pr create  # Add description with 🤖 signature
...
**After PR created:**
→ Use `/pr-iterate` to handle CI, feedback, and iterations
...
---
Note: This content duplicated in .cursor/rules/pr-create.mdc for auto-loading
